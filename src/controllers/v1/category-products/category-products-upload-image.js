...
const categoryProductsUploadImage = async (request, response) => {
    /*
    ...
    try {
        ...
        await ditokokuSequelize.transaction(async transaction => {
            try {
                ...
                query = "select category_products.id category_product_id, category_products.image_filename "+
                " from " + process.env.DB_DATABASE_DITOKOKU + ".category_products\n" +
                " where category_products.deleted_datetime is null and category_products.id = " + request.body['category_product_id'] +
                ";";
                const categoryProductImageData = await ditokokuSequelize.query(query, {transaction, type: QueryTypes.SELECT});

                if(categoryProductImageData[0].image_filename!==null){
                    unlink('public/assets/images/category-products/' + categoryProductImageData[0].image_filename, (err) => {
                        if (err) throw err;
                        console.log('file before was deleted');
                    });
                }

                const newpath = "public/assets/images/category-products/"+ request.body['file_name'];
        ...
        if (!error.hasOwnProperty('error_message')) {
            const errorJSON = {
                ...
                error_message: error.message + " (category product upload foto)",
                path: process.env.APP_BASE_URL + request.originalUrl
            };
...
export { categoryProductsUploadImage as default }