...
const adminsGet = async(request, response) =>{
    /*
    ...
    */
    ...
    try{
        ...
        if(request.query.filter!=null){
            ...
            if (filterOriginal.admin_id != null) {
                filterCondition = filterCondition + " and admins.id in ( " + filterOriginal.admin_id + " )";
                filters.push("admin_id in ( " + filterOriginal.admin_id + " )");
            }

            if (filterOriginal.admin_full_name != null) {
                filterCondition = filterCondition + " and ( "
                for (let counter = 0; counter < filterOriginal.admin_full_name.length; counter++) {
                    if (counter > 0) {
                        filterCondition = filterCondition + " or "
                    }
                    filterCondition = filterCondition + " admins.full_name like '%" + filterOriginal.admin_full_name[counter] + "%' ";
                }
                filterCondition = filterCondition + " ) ";
                filters.push("admin_full_name in ( " + filterOriginal.admin_full_name + " )");
            }

            if (filterOriginal.admin_username != null) {
                filterCondition = filterCondition + " and ( "
                for (let counter = 0; counter < filterOriginal.admin_username.length; counter++) {
                    if (counter > 0) {
                        filterCondition = filterCondition + " or "
                    }
                    filterCondition = filterCondition + " admins.username like '%" + filterOriginal.admin_username[counter] + "%' ";
                }
                filterCondition = filterCondition + " ) ";
                filters.push("admin_username in ( " + filterOriginal.admin_username + " )");
            }
        ...
        if (request.query.sort_by != null) {
            ...
            sortCondition = sortCondition + " order by admins.full_name ";
            sorts.push("admin_full_name");
        }
        ...
        //5 - Pengambilan total record
        const queryCount = "select count(admins.id) record_counts\n" +
        ...
        //6 - Ambil data dari database
        const query = "select admins.id admin_id, admins.username admin_username, admins.full_name admin_full_name\n" +
        ...
        const admins = await ditokokuSequelize.query(query,{ type: QueryTypes.SELECT });
