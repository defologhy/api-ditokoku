...
const bannersGet = async(request, response) =>{
    /*
    ...
    */
    ...
    try{
        ...
        if(request.query.filter!=null){
            ...
            if (filterOriginal.banner_id != null) {
                filterCondition = filterCondition + " and banners.id in ( " + filterOriginal.banner_id + " )";
                filters.push("banner_id in ( " + filterOriginal.banner_id + " )");
            }

            if (filterOriginal.banner_description != null) {
                filterCondition = filterCondition + " and ( "
                for (let counter = 0; counter < filterOriginal.banner_description.length; counter++) {
                    if (counter > 0) {
                        filterCondition = filterCondition + " or "
                    }
                    filterCondition = filterCondition + " banners.description like '%" + filterOriginal.banner_description[counter] + "%' ";
                }
                filterCondition = filterCondition + " ) ";
                filters.push("banner_description in ( " + filterOriginal.banner_description + " )");
            }
        ...
        if (request.query.sort_by != null) {
            ...
            sortCondition = sortCondition + " order by banners.id ";
            sorts.push("banner_id");
        }
        ...
        //5 - Pengambilan total record
        const queryCount = "select count(banners.id) record_counts\n" +
        ...
        //6 - Ambil data dari database
        const query = "select banners.id banner_id, banners.filename banner_filename, banners.description banner_description\n" +
        ...
        const banners = await ditokokuSequelize.query(query,{ type: QueryTypes.SELECT });
