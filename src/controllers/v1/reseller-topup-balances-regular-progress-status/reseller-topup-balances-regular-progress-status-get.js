...
const resellerTopupBalancesRegularProgressStatusGetGet = async(request, response) =>{
    /*
    ...
    */
    ...
    try{
        ...
        if (request.query.sort_by != null) {
            ...
            sortCondition = sortCondition + " order by rtbrps.created_datetime ";
        ...
        //5 - Pengambilan total record
        const queryCount = "select count(rtbrps.id) record_counts\n" +
            " from " + process.env.DB_DATABASE_DITOKOKU + ".reseller_topup_balances_regular_progress_status rtbrps\n" +
            " where rtbrps.deleted_datetime is null\n" +
            filterCondition +
            ";"
        ...
        //6 - Ambil data dari database
        const query = "select rtbrps.id progress_status_id, rtbrps.name progress_status_name\n"+
            "    , date_format(rtbrps.created_datetime,'%Y-%m-%d %H:%i:%s') created_datetime\n" +
            "    , date_format(rtbrps.last_updated_datetime,'%Y-%m-%d %H:%i:%s') last_updated_datetime\n" +
            "    , date_format(rtbrps.deleted_datetime,'%Y-%m-%d %H:%i:%s') deleted_datetime\n" +
            " from " + process.env.DB_DATABASE_DITOKOKU + ".reseller_topup_balances_regular_progress_status rtbrps\n" +
            " where rtbrps.deleted_datetime is null " +
        ...
        const resellerTopupBalancesRegularProgressStatusGet = await ditokokuSequelize.query(query,{ type: QueryTypes.SELECT });
        ...
        resultResponse = {
            ...
            , "data" : resellerTopupBalancesRegularProgressStatusGet
        }
...
export { resellerTopupBalancesRegularProgressStatusGetGet as default}